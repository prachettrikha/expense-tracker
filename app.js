  ...
  const html = sorted.map(([cat, amt]) => {
    ...
    return '<div class="budget-item">' +
             '<div class="budget-header">' +
               '<span class="cat-name"><span class="cat-icon">' + c.icon + '</span> ' + escHtml(cat) + '</span>' +
...
function renderPieChart() {
  ...
  sorted.forEach(([cat, amt]) => {
    ...
    // Legend item
    legendHtml += '<div class="legend-item' + (isActive ? ' active' : '') + '" data-cat="' + escHtml(cat) + '">' +
...
function handleFile(file) {
  ...
  reader.onload = e => {
    ...
    for (let i = 1; i < allRows.length; i++) {
      ...
      const cat  = col.cat !== -1 ? (c[col.cat] || 'Other').trim() : (learnedCategory(desc) || guessCategory(desc) || 'Other');
...
function renderPreview() {
  ...
  document.getElementById("previewList").innerHTML = parsedRows.map((r, i) => {
    ...
    return `<div class="preview-row ${r.isIncome ? "income" : ""}">
              ...
              <span class="pr-cat">${escHtml(mapped)}</span>
